...
export default function PaymentModal({ open, totalAmount, onCancel, onSubmit }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3 sm:p-4">
      {/* Card: fixed max height, header/footer pinned, middle scrolls — so
          Cancel/Submit are always reachable even when the QR content is tall. */}
      <div className="bg-brand-white rounded-2xl shadow-xl max-w-xl w-full max-h-[92vh] sm:max-h-[88vh]
                      flex flex-col border border-brand-light overflow-hidden">
        ...
        <div className="p-4 sm:p-6 pb-3 flex-shrink-0">
          ...
          <p className="text-sm text-brand-secondary">
            Send <strong className="text-brand-primary">{formatPHP(totalAmount)}</strong> using one of the methods below, then
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 min-h-0">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <div className="flex flex-col gap-2 w-full sm:w-36 sm:flex-shrink-0">
              ...
              >
                <div className="font-semibold text-sm text-brand-primary">GCash</div>
                <div className="text-xs text-brand-secondary mt-0.5">Scan QR to pay</div>
              </button>
              <button
                type="button"
              ...
              >
                <div className="font-semibold text-sm text-brand-primary">Bank Transfer</div>
                <div className="text-xs text-brand-secondary mt-0.5">UnionBank · Maribank · BPI</div>
              </button>

              {method === 'bank' && (
                <div className="flex flex-col gap-1.5 pt-1 border-t border-brand-light">
                  <p className="text-xs text-brand-secondary">Select bank:</p>
                    ...
                    <button
                      ...
                      className={`py-1.5 px-2 rounded-lg border text-xs font-medium text-left ${
                        bankOption === bank.id
                          ...
                          : 'border-brand-light text-brand-secondary'
                      }`}
                    >
                      {bank.label}
                    </button>
            ...
            {/* Right: QR code */}
            <div className="flex-1 flex flex-col items-center justify-center gap-2 p-3 rounded-lg bg-brand-bg border border-brand-light min-h-[200px]">
              {method === 'gcash' ? (
                <>
                  <p className="text-xs font-medium text-brand-primary">Scan to pay via GCash</p>
                  <button
                    type="button"
                    onClick={() => setPreviewSrc('/QRs/Gcash_QR.jpg')}
                    className="relative w-44 h-44 sm:w-56 sm:h-56 cursor-zoom-in rounded-lg overflow-hidden hover:opacity-90 transition-opacity"
                ...
                <>
                  <p className="text-xs font-medium text-brand-primary">Scan to pay via {selectedBank.label}</p>
                  <button
                    type="button"
        ...
        {/* Footer (pinned) */}
        <div className="p-4 sm:p-6 pt-3 border-t border-brand-light flex-shrink-0">
        ...
        >
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              type="button"
              onClick={() => setPreviewSrc(null)}
              className="absolute -top-3 -right-3 bg-white rounded-full w-7 h-7 flex items-center justify-center text-brand-primary shadow text-sm font-bold hover:bg-brand-bg"
            >
              ✕
            </button>
          </div>
